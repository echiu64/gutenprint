  ...
  for (i = 0; i < steps; i++)
    {
      ...
      unsigned base;
      /* MRS - THIS SHOULD BE CHANGED TO ROW MAJOR FOR BETTER CACHE PERFORMANCE!!!!!!! */
      base = map[ya + (xa * size)];
      div1 = 1;
      for (j = i; j < steps - 1; j++)
	div1 *= size * size;
	    ...
	    int vertical_aspect, const char *dither_algorithm)
{
  ...
  if (!strcmp(dither_algorithm, "Hybrid Floyd-Steinberg"))
    d->dither_type = D_FLOYD_HYBRID;
  else if (!strcmp(dither_algorithm, "Random Floyd-Steinberg"))
    d->dither_type = D_FLOYD;
  else if (!strcmp(dither_algorithm, "Ordered"))
    d->dither_type = D_ORDERED;
  else if (!strcmp(dither_algorithm, "Adaptive Hybrid"))
    d->dither_type = D_ADAPTIVE_HYBRID;
  else if (!strcmp(dither_algorithm, "Adaptive Random"))
...
void
dither_fastblack(int           	    row,	/* I - Current Y coordinate */
		 void 		    *vd,
		 unsigned char 	    *black)	/* O - Black bitmap pixels */
{
  int		x;		/* Current X coordinate */
  unsigned char	*kptr;		/* Current black pixel */
  dither_t *d = (dither_t *) vd;
  ...
  {
    if (*kptr > 127)
      *kptr++ = d->density >= ditherpoint(d, &(d->k_dithermat), x, row);
    else
      *kptr++ = 0;
  }
}
...
void
dither_black(int           	row,		/* I - Current Y coordinate */
	     void 		*vd,
	     unsigned char 	*black)		/* O - Black bitmap pixels */
{
  int		x;		/* Current X coordinate */
	    ...
	    int *nc, int *nm, int *ny, int *nk, int *jk)
{
  ...
  /**** MRS - Should magenta be *3 or more? ****/
...
void
dither_cmyk(int           row,	/* I - Current Y coordinate */
	    void 	  *vd,
	    ...
	    unsigned char *black)	/* O - Black bitmap pixels */
{
  ...
  int		c, m, y, k,	/* CMYK values */
		oc, om, oy;
  unsigned char	*cptr,		/* Current cyan pixel */
  ...
  if (direction == -1)
      ...
      if (! (d->dither_type & D_ORDERED_BASE))
	{
      ...
      cptr = cyan + d->dst_width - 1;
      if (lcptr)
	lcptr = lcyan + d->dst_width - 1;
      mptr = magenta + d->dst_width - 1;
      if (lmptr)
	lmptr = lmagenta + d->dst_width - 1;
      yptr = yellow + d->dst_width - 1;
      if (lyptr)
	lyptr = lyellow + d->dst_width - 1;
      if (kptr)
	kptr = black + d->dst_width - 1;
  ...
  for (; x != terminate; x += direction)
...
#if 0
      /**** MRS - this part should be in the RIP and CM portion ****/
      /*
      ...
      if (k > 0)
	{
       ...
       * k = raw K value derived from CMY or generated by RIP
       * oc, om, oy = raw CMY values including K component
