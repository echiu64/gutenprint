...
main(int argc, char **argv)
{
  ...
  printf("#endif\n");
  printf("#include <gimp-print/gimp-print.h>\n");
  printf("#include \"gimp-print-internal.h\"\n");
  printf("#include <gimp-print/gimp-print-intl-internal.h>\n");
  printf("#include \"vars.h\"\n\n");
  printf("\n");
  printf("typedef struct stp_internal_printer\n");
  printf("{\n");
  printf("  const char *long_name,	/* Long name for UI */\n");
  printf("             *driver;	/* Short name for printrc file */\n");
  printf("  int        model;			/* Model number */\n");
  printf("  const stp_printfuncs_t *printfuncs;\n");
  printf("  stp_internal_vars_t printvars;\n");
  printf("} stp_internal_printer_t;\n");
  printf("\n");
  for (i = 0; i < nprintfuncs; i++)
  ...
  printf("};\n\n");
  printf("int\n");
  printf("stp_known_printers(void)\n");
  printf("{\n");
  printf("  return sizeof(stp_printer_list) / sizeof(stp_internal_printer_t);\n");
  printf("}\n\n");
  printf("const stp_printer_t\n");
  printf("stp_get_printer_by_index(int idx)\n");
  printf("{\n");
  printf("  if (idx < 0 || idx >= stp_known_printers())\n");
  printf("    return NULL;\n");
  printf("  return (stp_printer_t) &(stp_printer_list[idx]);\n");
  printf("}\n");
  printf("\n");
  printf("const char *\n");
