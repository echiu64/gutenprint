...
void output_paper(stpi_internal_papersize_t *paper);
...
void stpi_xml_process_gimpprint(xmlNodePtr gimpprint, int mode);
void stpi_xml_process_printdef(xmlNodePtr printdef);
void stpi_xml_process_family(xmlNodePtr family);
void stpi_xml_process_printer(xmlNodePtr printer, xmlChar *family);
void stpi_xml_process_paperdef(xmlNodePtr paperdef);
stpi_internal_papersize_t *stpi_xml_process_paper(xmlNodePtr paper);
xmlNodePtr stpi_xml_get_node(xmlNodePtr prop, const xmlChar *name);
...
#endif
  stpi_xml_process_gimpprint(cur, mode);
...
void printer_output_start(void)
{
  ...
  while(family_names[i])
    {
      fprintf(stdout, "const extern stpi_printfuncs_t stp_%s_printfuncs;\n",
...
void paper_output_start(void)
{
  fputs("/* This file is automatically generated.  See papers.xml.\n"
	"   DO NOT EDIT! */\n\n",
	stdout);
  fputs("\nstatic stpi_internal_papersize_t paper_sizes[] =\n"
	"{\n",
	stdout);
}
...
void
stpi_xml_process_gimpprint(xmlNodePtr cur, int mode)
{
  xmlNodePtr child;
  child = cur->children;
  while (child)
...
void
stpi_xml_process_printdef(xmlNodePtr printdef)
{
  xmlNodePtr family;
...
void
stpi_xml_process_family(xmlNodePtr family)
{
  xmlChar *family_name;
  xmlNodePtr printer;
  ...
  while (family_valid && printer)
	...
	stpi_xml_process_printer(printer, family_name);
...
xmlNodePtr
stpi_xml_get_node(xmlNodePtr prop, const xmlChar *name)
{
  while (prop)
    {
...
void
stpi_xml_process_printer(xmlNodePtr printer, xmlChar *family)
{
  xmlNodePtr prop;
  ...
  printf("    \"%s\",\n", (const char *) family);
  prop = stpi_xml_get_node(printer->children, (const xmlChar *) "model");
  printf("    %d,\n",
	 xmlstrtol(xmlGetProp(prop, (const xmlChar *) "value")));
  prop = stpi_xml_get_node(printer->children, (const xmlChar *) "color");
  printf("    %d,\n",
...
void
stpi_xml_process_paperdef(xmlNodePtr printdef)
{
  xmlNodePtr paper;
  ...
  while (paper)
	...
	{
	  output_paper(stpi_xml_process_paper(paper));
	}
      paper = paper->next;
    }
}
...
stpi_xml_process_paper(xmlNodePtr paper)
{
  xmlNodePtr prop;
  ...
  static stpi_internal_papersize_t outpaper;
