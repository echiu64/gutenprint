...
In Mac OS 10.2, the system *always* describes any printer driven by Gimp-Print as a PostScript device, which means that a PostScript interpreter (such as GhostScript) is required by the Gimp-Print driver to print from this class of application.
</p><p>
In Mac OS 10.3, by default the system *always* describes any printer driven by Gimp-Print as a raster device, which is technically correct. The reason why your EPS files looked 'jaggedy' in 10.3 has to do with the quality of raster generated by your application. You can return the behavior of 10.3 to that of 10.2 by setting a specific 'system default'. Here is the procedure to do that:</p>
<ol>
<li> Quit your application, if it is running</li>
<li> open the Terminal application found in /Applications/Utilities/</li>
<li> copy the following command in its entirety and paste it into the Terminal window, then press the return key:</li>
</ol>
<pre>
...
<p>
This setting is permanent. If you wish to return to the default behavior use this command:
<pre>
